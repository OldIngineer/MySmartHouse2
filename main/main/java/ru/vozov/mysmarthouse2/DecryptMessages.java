...
// соглано выбранным сценариям,
    ...
    public byte[] decryptScriptClimate(boolean[] mCheckActiveModule, String[] mTempCurrent,
            String[] mTempIn, String[] mTempOut, byte[][] mStateObject, byte[] mControlBoiler)
    {
        byte[] com = {0,6,0,0,0,0,1,1};
        //проверка сценария включения отопления
        for (int i = 1; i< mCheckActiveModule.length; i++)//перебор модулей
        {   //если модуль активный и включен режим "КЛИМАТ"
            if ((mCheckActiveModule[i]) & (mControlBoiler[2] == 1)) {
                //если модуль не подключен к котлу, подключен датчик и
                // значения температур включения/выключения отопления выбраны
                if ((mControlBoiler[0]!=i)&(!mTempCurrent[i].equals("н/д"))&
                        (!mTempIn[i].equals("no"))&(!mTempOut[i].equals("no"))) {
                ...
                //если модуль выбран в качестве менеджера котла, подключен датчик и
               // значения температур включения/выключения отопления выбраны
                if ((i == mControlBoiler[1])&(!mTempCurrent[i].equals("н/д"))&
                        (!mTempIn[i].equals("no"))&(!mTempOut[i].equals("no"))) {
                 //включить/выключить отопление в помещении где расположен узел
                    if ((Byte.parseByte(mTempCurrent[i]) < Byte.parseByte(mTempIn[i]))
                            & (mStateObject[i][4] == 0)) {
                        com[0] = (byte) i;//адрес модуля
                        com[4] = 64;//включить
                 ...
                 //включить/выключить котел
                    if ((Byte.parseByte(mTempCurrent[i]) < Byte.parseByte(mTempIn[i]))
                            & (mStateObject[mControlBoiler[0]][4] == 0)) {
                        com[0] = mControlBoiler[0];//адрес модуля к которому подключен котел
                        com[4] = 64;//включить
                        com[5] = 16;//отопление
                    }
                    if ((Byte.parseByte(mTempCurrent[i]) > Byte.parseByte(mTempOut[i]))
                            & (mStateObject[mControlBoiler[0]][4] == 1)) {
                        com[0] = mControlBoiler[0];//адрес модуля к которому подключен котел
                        com[4] = 0;//выключить
                        com[5] = 16;//отопление
                    }
